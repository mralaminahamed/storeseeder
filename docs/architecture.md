...
│   │   └── Generators/          #   21 concrete generators, one per resource
│   ├── Rest/                    # REST surface
...
│       ├── Resource.php            # the 21 canonical resource names
...
│           │   └── Writers/        # 20 writers — tags have no Fluent Cart taxonomy
│           └── Woo_Commerce/
│               ├── Platform.php    # matrix: 18 supported, 3 refused with reasons
│               ├── Writer.php      # shared money, status and foreign-key helpers
│               └── Writers/        # 18 writers, through WooCommerce's CRUD objects
...
| `storeseeder_platform_fields_{id}` | filter | One platform's extra generation parameters for one resource. The field-level counterpart of `storeseeder_platform_supports_{id}`: how an extension that adds a column to its platform exposes it for seeding. Entries that are not a schema array, or are numerically keyed, are discarded rather than handed to `register_rest_route` |
