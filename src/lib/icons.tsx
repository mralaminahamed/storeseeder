...
/**
 * The admin's icon set, as inline path data on a 24-unit grid.
 *
 * The artwork is Lucide (https://lucide.dev), ISC licensed, copyright the Lucide
 * contributors; each entry is the path data of the icon it is named after. They
 * are inlined rather than imported so the admin bundle carries no icon library —
 * one `<Icon name="…">` component draws all of them, and unused glyphs cost a
 * line of this file rather than a module in the graph.
 *
 * Keys are flat lowercase because they travel as strings through
 * `Generator.iconName` and the command palette. A name with no entry falls back
 * to `box`, so check `ICONS` before referring to a new one — four generator icons
 * silently rendered a plain box for exactly that reason.
 */
const ICONS = {
  ...
  sliders: '<path d="M21 4h-7"/><path d="M10 4H3"/><path d="M21 12h-9"/><path d="M8 12H3"/><path d="M21 20h-5"/><path d="M12 20H3"/><circle cx="12" cy="4" r="2"/><circle cx="10" cy="12" r="2"/><circle cx="14" cy="20" r="2"/>',
  ...
  // A single counter-clockwise arrow. `refresh` is two arrows in a loop and reads as "sync";
  // undoing is one direction, and the difference matters on a destructive button.
  ...
  sparkles: '<path d="M11.52 2.3a.5.5 0 0 1 .96 0l1.58 6.14A2 2 0 0 0 15.5 9.94l6.14 1.58a.5.5 0 0 1 0 .96L15.5 14.06a2 2 0 0 0-1.44 1.44l-1.58 6.14a.5.5 0 0 1-.96 0l-1.58-6.14A2 2 0 0 0 8.5 14.06L2.36 12.48a.5.5 0 0 1 0-.96L8.5 9.94A2 2 0 0 0 9.94 8.5Z"/>',
  ...
  chart: '<path d="M3 3v16a2 2 0 0 0 2 2h16"/><path d="M7 15v-3"/><path d="M12 15V9"/><path d="M17 15v-6"/>',
  ...
  // Generator marks. Four of these were referenced by lib/generators.ts before
  // they existed here, so `ICONS[name] ?? ICONS.box` rendered a plain box for
  // Shipping Classes, Labels, Order Tax Lines and Subscriptions.
  ...
  // A storefront, for the e-commerce platform data is written to. `boxes` used to
  // carry that, which put the same glyph on the platform selector and on the
  // Shipping Classes generator.
  ...
  // Appearance. `sun` reads as "light mode" specifically, which is wrong on a card
  // that also holds the dark option.
  palette: '<path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.648-.746 1.648-1.688 0-.437-.18-.835-.437-1.125-.29-.289-.438-.652-.438-1.125a1.64 1.64 0 0 1 1.668-1.668h1.996c3.051 0 5.555-2.503 5.555-5.554C21.965 6.012 17.461 2 12 2z"/><circle cx="13.5" cy="6.5" r=".5" fill="currentColor"/><circle cx="17.5" cy="10.5" r=".5" fill="currentColor"/><circle cx="8.5" cy="7.5" r=".5" fill="currentColor"/><circle cx="6.5" cy="12.5" r=".5" fill="currentColor"/>',
  // A licence key. Lucide's key-round: the conventional glyph for licensing, and distinct
  // from `ticket`, which Coupons already uses — two concepts must not share one mark.
  key: '<circle cx="7.5" cy="15.5" r="5.5"/><path d="m21 2-9.6 9.6"/><path d="m15.5 7.5 3 3L22 7l-3-3"/>',
  // Growth, for a positive week-on-week delta. A bar chart says "statistics", not
  // "up".
  trend: '<polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/>',
} satisfies Record<string, string>;

/**
 * Every name the set knows. Typing callers with this makes an unregistered name a
 * compile error rather than a box at runtime.
 */
