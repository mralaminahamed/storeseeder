...
| 15 | Prefixing | ✅ | All globals prefixed `fluent_cart_fakerpress` / `FluentCartFakerPress` / `FLUENT_CART_FAKERPRESS`; 24 hook names verified. |
| 16 | Text domain | ✅ | 289 i18n calls, all `fluent-cart-fakerpress`; POT present. |
...
2. `composer zip:dev` or `composer release` and confirm the zip contains `build/`, `vendor/`, `includes/`, `languages/`, `readme.txt`, `fluent-cart-fakerpress.php` — and **not** `src/`, `tests/`, `node_modules/`, `.wordpress-org/`.
